...
let webPart : WebPart<HttpContext> =
    choose [
        GET >=> path "/api/movies" >=> write OK movies
        GET >=> path "/api/movies2" >=> write OK movies
        GET >=> pathScan "/movies/%s/player" (fun f -> OK (page (sprintf "http://127.0.0.1:8085/movies/%s" f))) >=> asHtml
        GET >=> pathScan "/movies/%s" (fun f -> partialFile <| Directory.GetFiles(folder, sprintf "%s.*" f).[0])
    ]
